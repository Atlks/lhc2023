...
//gene rzt list pic
  ...
  //  return "";
    ...
    \think\facade\Log::debug($lineNumStr);
    $total_payout = 0;
    \think\facade\Log::debug("bef getKaijNumFromBlkhash:" . $gameNo);
    require_once __DIR__ . "/lotrySscV2.php";
    ...
    var_dump($this->lottery_no);
    //select from  BetRecord
    $records = Logs::getBetRecordByLotteryNo($gameNo);
    //var_dump( $records);
    var_dump(count($records));
    $players = [];    //zhonjyo wanja
    // bet_records需要跟新字段 Payout,Status,ResultId,Rebate
    ...
    foreach ($records as $k => $v) :
      try {
        ...
        $rzt_dwojyo = betstrX__compare_dwijyo_bjl($betContext, $result_text);
        //  dwijyo($betContext, $kaij_num);
        if (!$rzt_dwojyo) {
          \think\facade\Log::info("dwijyuo rzt false");
          var_dump("dwijyuo rzt false");
    ...
    $text = $text . jonjyo_bingoList_calcIncomeGrpby($gameNo);
  ...
  private function getBlsByU($uid) {
